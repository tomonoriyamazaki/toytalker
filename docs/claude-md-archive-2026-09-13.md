# CLAUDE.md 退避版（2026-09-13圧縮前の全文）

2026-09-13にCLAUDE.mdを「概要・ルール・現在の状態」だけへ圧縮した際、日付付きの経過を失わないよう圧縮前の全文をそのまま退避したもの。現在の状態はルートの [CLAUDE.md](../CLAUDE.md)、各件の詳細は本文中でリンクされた調査メモを参照する。ここは更新しない。
