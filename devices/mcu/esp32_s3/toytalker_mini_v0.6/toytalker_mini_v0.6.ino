...
constexpr bool VOICE_BARGE_DETECT_ONLY = true;   // 再生音で誤発火したため、まず停止せず計測
...
constexpr uint32_t VOICE_BARGE_RMS = 6000;       // 未校正。実機ログで調整
...
void voiceMonitorTask(void*) {
  int32_t raw[320];  // 20ms @ 16kHz。毎ターンのheap確保なし
  for (;;) {
    ...
    while (voiceMonitorEnabled.load()) {
      ...
      if (millis() - lastLog >= 1000) {
        Serial.printf("[VOICE] rms=%lu max=%lu threshold=%lu armed=%d frames=%lu read_errors=%lu\n",
