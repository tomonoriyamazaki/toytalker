...
- ブランチは `feature/firmware-v0.6-voice-barge-in`。今回の保存前のHEADは `18756d0`（認証チェックリスト・運用費用資料）。`87035d8`（Lambdaの検索後の前置き抑制）、`d3a59a8`（AEC誤停止対策・毎ターン再初期化）もコミット済み。
- 今回の保存内容はv0.7の②相槌通知修正と、`CLAUDE.md`、`docs/esp32-v07-aec.md`、新規の `docs/esp32-v07-latency-review.md`、`docs/esp32-ota-settings-plan.md` の引き継ぎ記録。ファームはビルド済み、通常会話での通知値と効果の対応は実機確認途中。コミット名は `Fix backchannel hint before playback and document handoff`。再開時は `git status` / `git log` で最新状態を確認する。
